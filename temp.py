...
def get_file_names(dsn):
    ...
    os.chdir("/Users/brodriguez/Documents/Owlet-Code")
    file_names = []
...
def get_source(df, names):
    ...
    # Dots on xaxis instead of horizontal lines:
    # source_data5 = {'red' : [df.iloc[i].timestamp] for i in red}
    # source_data5.update({'red_ys' : [1.5 for i in red]})
...
def set_up_widgets(p, source, source1, source2, source3, source4, source5, df, all_dates, text_dsn):
    ...
    min_day = date_of_file(all_dates[0]) # works if sorted, needs to be formatted...
